...
def create_report():
    ...
    add_h2("4.3 MICROSERVICE API ENDPOINT DEFINITIONS & REQUEST/RESPONSE CONTRACTS")
    add_p("The NestJS API Gateway exposes RESTful endpoints for querying spatial building hierarchies, historical sensor metrics, and active alert logs. Key REST endpoints serving the 3D web frontend include GET /api/v1/buildings, GET /api/v1/floors/:id/assets, GET /api/v1/sensors/:id/readings, and POST /api/v1/simulation/scenario.")

    add_h2("4.4 INGESTION PIPELINE FAILOVER & CIRCUIT BREAKER ARCHITECTURE")
    add_p("To ensure high availability during network partitions or database maintenance restarts, the ingestion worker incorporates an in-memory circuit breaker pattern. If disk write operations to TimescaleDB experience transient timeouts, the worker buffers incoming telemetry packets in Valkey RAM queues for up to 15 minutes. Once database connectivity is restored, the buffered messages are written sequentially without data loss.")
    ...
    # CHAPTER 12: APPENDIX (15 REAL MONOREPO CODE MODULES)
    ...
    code_modules = [
        ...
        ("9. NestJS Telemetry REST Controller (apps/api-gateway/src/domains/telemetry/telemetry.controller.ts)", "apps/api-gateway/src/domains/telemetry/telemetry.controller.ts"),
        ("10. FastAPI Copilot AI Routing (apps/ai-service/routers/copilot.py)", "apps/ai-service/routers/copilot.py"),
        ("11. Shared Monorepo Domain Types (packages/shared/src/types.ts)", "packages/shared/src/types.ts"),
        ("12. 3D Digital Twin Viewer Canvas Component (apps/web/src/components/viewer/Building3D.tsx)", "apps/web/src/components/viewer/Building3D.tsx"),
        ("13. Live Telemetry Monitor UI Component (apps/web/src/components/telemetry/TelemetryMonitor.tsx)", "apps/web/src/components/telemetry/TelemetryMonitor.tsx"),
        ("14. Real-Time Alert Manager Component (apps/web/src/components/alerts/AlertManager.tsx)", "apps/web/src/components/alerts/AlertManager.tsx"),
        ("15. Conversational AI Copilot Interface (apps/web/src/components/copilot/AiCopilotChat.tsx)", "apps/web/src/components/copilot/AiCopilotChat.tsx")
    ]
...
if __name__ == '__main__':
    ...
    print(f"=== CLEAN EXPANDED DOCUMENT GENERATED: MS WORD PAGE COUNT = {pages} PAGES ===")
