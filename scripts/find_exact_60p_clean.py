...
def build_with_pair(path1, title1, path2, title2):
    doc = docx.Document()
...
if __name__ == '__main__':
    p1 = "apps/web/src/features/digital-twin/building-geometry.ts"
    t1 = "9. 3D Building Geometry Definitions (apps/web/src/features/digital-twin/building-geometry.ts)"
    
    candidates = [
        ("apps/web/src/hooks/useSensorRealtime.ts", "10. React Sensor Real-Time Ingestion Hook (apps/web/src/hooks/useSensorRealtime.ts)"),
        ("apps/api-gateway/src/sensors/sensors.service.ts", "10. NestJS Sensors Service Implementation (apps/api-gateway/src/sensors/sensors.service.ts)"),
        ("apps/web/src/hooks/useRealtime.ts", "10. Web Application Realtime Hook (apps/web/src/hooks/useRealtime.ts)"),
        ("apps/api-gateway/src/buildings/buildings.service.ts", "10. NestJS Buildings Service (apps/api-gateway/src/buildings/buildings.service.ts)")
    ]
    
    for path2, title2 in candidates:
        if os.path.exists(path2):
            doc_path = build_with_pair(p1, t1, path2, title2)
            pages = measure_word_pages(doc_path)
            print(f"Testing Pair '{path2}': MS Word Page Count = {pages} Pages")
            if pages == 60:
                print(f"=== EXACT 60 PAGES ACHIEVED IN MS WORD WITH PAIR '{path2}'! ===")
                break
